...
const GUN_TYPES = Object.fromEntries(
  Object.entries(WEAPON_CATEGORIES).flatMap(([catName, catData]) => {
...
function damagePerOneShot(stats = {}, damageKey = "damage_max", body = "base", pelletOverride = null) {
  // Per one shot damage = pellets * (base damage * body multiplier)
  const base = toNumber(stats[damageKey]);
  if (base == null) return null;
  const pellets = pelletOverride != null ? pelletOverride : toNumber(stats.pellet_count) || 1;
  const mult =
...
const computeDPSDetails = (stats) => {
  ...
  return {
    dios: perShotFull != null ? perShotFull * limitedShots : null,
    ...
    switch (key) {
      ...
      case "recoil_combined": {
        const v = toNumber(s.vertical_recoil),
          h = toNumber(s.horizontal_recoil);
...
function renderGunCard(name) {
  ...
  STATS.forEach((s) => {
    if (s.type === "separator") {
      statsEl.appendChild(el("div", "stat-separator"));
      return;
    }
...
function renderComparison(w1, w2, s1, s2) {
  ...
  const colA = nodeA.querySelector(".comparison-weapon");
  colA.querySelector("h4").textContent = w1;
  const colB = nodeB.querySelector(".comparison-weapon");
  colB.querySelector("h4").textContent = w2;
...
function populateWeaponDropdowns() {
  ...
  s1.className = elements.weapon1Select.className;
  ...
  s2.className = elements.weapon2Select.className;
